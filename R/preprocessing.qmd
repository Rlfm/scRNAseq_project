...
ElbowPlot(retina.scrna.no_dbl)
```

# Unsupervised Clustering & Evaluation

```{r}
retina.scrna.no_dbl <- FindNeighbors(retina.scrna.no_dbl, dims=1:18)
retina.scrna.no_dbl <- FindClusters(
  retina.scrna.no_dbl, resolution = seq(0.1, 0.5, 0.1))

clustree(retina.scrna.no_dbl, prefix = "SCT_snn_res.")
```

# Dim. Reduction via UMAP

```{r}
retina.scrna.no_dbl <- RunUMAP(retina.scrna.no_dbl, dims = 1:18)
```

```{r}
# change identities to best cluster resolution
Idents(retina.scrna.no_dbl) <- "SCT_snn_res.0.3"
```

```{r}
DimPlot(retina.scrna.no_dbl, group.by = "bc3_well")
```
