...
```{r}
DimPlot(retina.scrna.no_dbl, group.by = "SCT_snn_res.0.2")
```
...
```{r}
retina.scrna.no_dbl_ASL1 <- subset(retina.scrna.no_dbl, subset = idents == "6")
```

```{r}
saveRDS(retina.scrna.no_dbl, "../data/retina.preprocessed_ASL1.rds")
```
